  ...
  <div class="taovien">
    <?php
		...
		function listCatShow($cat,$sau,$webRoot,$languageMantan)
		{
			...
			?>
			<img src="<?php echo $webRoot;?>images/bg-list-item.png" />&nbsp&nbsp<a target="_blank" href="<?php echo getUrlVideoCategory($cat['id'],$cat['slug']);?>"><span id="name<?php echo $cat['id'];?>"><?php echo $cat['name'];?></span></a>
	...
	?>
			...
			<form name="dangtin" method="post" action="<?php echo $urlOptions;?>saveCategoryVideo" role="form">
...
function deleteData(idDelete,slugDelete)
{
	...
	if(check)
	{
		$.ajax({
	      type: "POST",
	      url: urlWeb+"deleteCategoryVideo",
	      data: { idDelete:idDelete,slugDelete:slugDelete}
	    }).done(function( msg ) { 	
		  		window.location= urlWeb+"categoryVideo";	
		 })
		 .fail(function() {
				window.location= urlWeb+"categoryVideo";
			});  
...
function diChuyen(type, idMenu)
{
	$.ajax({
      type: "POST",
      url: urlWeb+"changeCategoryVideo",
