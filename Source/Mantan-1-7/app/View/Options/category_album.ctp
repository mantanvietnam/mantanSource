...
<?php
	$breadcrumb= array( 'name'=>$languageMantan['album'],
						'url'=>$urlOptions.'categoryAlbum',
						'sub'=>array('name'=>$languageMantan['categories'])
					  );
  ...
  <div class="taovien">
    <?php
		...
		function listCatShow($cat,$sau,$webRoot,$languageMantan)
		{
			...
			?>
			<img src="<?php echo $webRoot;?>images/bg-list-item.png" />&nbsp&nbsp<a target="_blank" href="<?php echo getUrlAlbumCategory($cat['id'],$cat['slug']);?>"><span id="name<?php echo $cat['id'];?>"><?php echo $cat['name'];?></span></a>
	...
	?>
			...
			<form name="dangtin" method="post" action="<?php echo $urlOptions;?>saveCategoryAlbum" role="form">
...
function deleteData(idDelete,slugDelete)
{
	...
	if(check)
	{
		$.ajax({
	      type: "POST",
	      url: urlWeb+"deleteCategoryAlbum",
	      data: { idDelete:idDelete,slugDelete:slugDelete}
	    }).done(function( msg ) { 	
		  		window.location= urlWeb+"categoryAlbum";	
		 })
		 .fail(function() {
				window.location= urlWeb+"categoryAlbum";
			});  
...
function diChuyen(type, idMenu)
{
	$.ajax({
      type: "POST",
      url: urlWeb+"changeCategoryAlbum",
