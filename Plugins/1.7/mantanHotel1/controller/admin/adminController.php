...
function addManager($input) {
    ...
    if (checkAdminLogin()) {
        if ($isRequestPost) {
            ...
            if ($modelManager->isExist($dataSend['user'], $dataSend['email']) == true) {
                $mess = 'Tài khoản hoặc email đã tồn tại!';
                ...
                if ($modelManager->save($save)) {
                    ...
                    $to = array(trim($save['Manager']['email']), trim($contactSite['Option']['value']['email']));
                    $cc = array();
                    $bcc = array();
        ...
        if ($data) {
            ...
            if ($isRequestPost) {
                ...
                if ($modelRequest->updateAll($save, array('_id' => $idRequest))) {
                    ...
                    // send email for user and admin
                    $from = array($contactSite['Option']['value']['email'] => $smtpSite['Option']['value']['show']);
                    $to = array(trim($data['Request']['email']), trim($contactSite['Option']['value']['email']));
                    $cc = array();
                    $bcc = array();
