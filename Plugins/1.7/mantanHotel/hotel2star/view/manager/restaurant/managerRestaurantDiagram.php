...
<script type="text/javascript">
    var urlPaid = "<?php echo $urlHomes; ?>managerCheckoutRestaurant";
    var urlEdit = "<?php echo $urlHomes; ?>managerAddTableRestaurant";
    var urlClear = "<?php echo $urlHomes; ?>managerClearTableRestaurant";
    var urlDelete = "<?php echo $urlHomes; ?>managerDeleteTableRestaurant";
    var urlEditcost = "http://";
    var urlChangeroom = "<?php echo $urlHomes; ?>managerChangTableRestaurant";
    var urlAddservice = "<?php echo $urlHomes; ?>managerAddServiceTableRestaurant";
    var urlCreatebill = "";
    var urlReceived = "<?php echo $urlHomes; ?>managerCheckinRestaurant";
    var urlCancel = "<?php echo $urlHomes; ?>managerCancelCheckinRestaurant";
    var url;
    ...
    var urlViewroomdetail = "<?php echo $urlHomes; ?>managerViewTableRestaurantDetail";
    ...
    $(function () {
        ...
        $.contextMenu({
            ...
            items: {
                ...
                "sep1": "---------",
//                "listwaiting": {name: "Danh sách khách chờ", icon: "list", accesskey: "l w"},
...
<div id="dialog">
    Chọn nhà hàng chuyển đến
    <select id="selecTo">
        <?php
        foreach ($listRestaurantFloor as $floor) {
            echo '<option value="0" style="">' . $floor['RestaurantFloor']['name'] . '</option>';
            foreach ($listTables[$floor['RestaurantFloor']['id']] as $infoTable) {
                if (empty($infoTable['Table']['checkin'])) {
                    echo '<option value="' . $infoTable['Table']['id'] . '">|--' . $infoTable['Table']['name'] . '</option>';
                }
            }
        }
        ?>
...
<section role="main" class="content-body">
    <header class="page-header">
        ...
        <div class="right-wrapper pull-right">
            <ol class="breadcrumbs">
                ...
                <li><span>Sơ đồ nhà hàng</span></li>
            </ol>
    ...
    <section class="panel">
        <header class="panel-heading">
            ...
            <h2 class="panel-title">Sơ đồ nhà hàng</h2>
        </header>
            ...
            foreach ($listRestaurantFloor as $floor) {
                echo ' 
					<div class="row diagram">
						<div style="background-color: ' . $floor['RestaurantFloor']['color'] . ';" class="col-md-1 col-sm-2 floors context-menu-three" idRestaurantFloor="' . $floor['RestaurantFloor']['id'] . '">' . $floor['RestaurantFloor']['name'] . '</div>
                ...
                foreach ($listTables[$floor['RestaurantFloor']['id']] as $infoTable) {
                    if (empty($infoTable['Table']['checkin'])) {
                        ...
                        if (isset($infoTable['Table']['clear']) && $infoTable['Table']['clear'] == FALSE) {
                            $classColor = 'un-clear';
                        } else {
                        ...
                        echo '
                                <div class="col-sm-4 col-md-2 ' . $classColor . ' context-menu-two" idtable="' . $infoTable['Table']['id'] . '" nameroom="' . $infoTable['Table']['name'] . '">
                                        <div class="name-room"></div>
                                        <div class="customer-name"><span class="room-number">' . $infoTable['Table']['name'] . '</span></div>
                                </div>
                        ';
                    } else {
                        
                        $classColor = 'booked';
                        $dateCheckin = date("d/m", $infoTable['Table']['checkin']['dateCheckin']);
                        $today = time();
                        $tru = $today - $infoTable['Table']['checkin']['dateCheckin'];
                        $showTime = round($tru / 3600) . ' giờ';
//                        if ($infoTable['Table']['checkin']['type_register'] == 'gia_theo_ngay')
//                            $showTime = round($tru / 86400) . ' ngày';
//                        else
//                            $showTime = round($tru / 3600) . ' giờ';
                        //debug ($tru);
                        echo '
                                <div class="col-sm-4 col-md-2 ' . $classColor . ' context-menu-one" idtable="' . $infoTable['Table']['id'] . '" nameroom="' . $infoTable['Table']['name'] . '">
                                    <div class="name-room"></div>
                                    <div class="time">';
//                        if ($infoTable['Table']['checkin']['dateCheckoutForesee'] > 0)
//                            echo '
...
//                                        </div>';
                        echo '<span> ' . $showTime . ' <br/>' . $dateCheckin . '</span>
                                    ...
                                    <div class="customer-name"><span class="room-number">' . $infoTable['Table']['name'] . '</span>  <br/><span>(' . $infoTable['Table']['checkin']['number_people'] . ') ' . @$infoTable['Table']['checkin']['cus_name'] . '</span></div>
                                </div>
                            ';
                    }
                }
